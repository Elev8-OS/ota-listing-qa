  ...
  try {
    await withBrowser(async (page) => {
      ...
      result.note = result.ok
        ...
        : `Auf der öffentlichen Airbnb-Seite konnten keine der bekannten Felder gefunden werden (Seitenstruktur evtl. geändert). Bitte manuell erfassen. [Diagnose: URL=${page.url()} Titel="${title}" Textanfang="${bodyText.slice(0, 180).replace(/\s+/g, " ")}"]`;
    });
  } catch (err) {
  ...
  try {
    await withBrowser(async (page) => {
      ...
      result.note = result.ok
        ...
        : `Auf der öffentlichen Booking.com-Seite konnten keine der bekannten Felder gefunden werden (Seitenstruktur evtl. geändert). Bitte manuell erfassen. [Diagnose: URL=${page.url()} Titel="${title}" Textanfang="${bodyText.slice(0, 180).replace(/\s+/g, " ")}"]`;
    });
  } catch (err) {
